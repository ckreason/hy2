...
# 获取该子域名的解析 IP
RESOLVED_IP=$(dig +short "$SELECTED_DOMAIN" | grep -E '^[0-9.]+' | head -n 1)
...
sleep \$((RANDOM % 30 + 10))
cd "$WORKDIR"
nohup ./hysteria2 server -c web.yaml > hy2.log 2>&1 &
EOF
...
SUB_URL="hysteria2://$PASSWORD@$RESOLVED_IP:$udp_port/?sni=$MASQUERADE_DOMAIN&alpn=h3&insecure=1#$TAG"
